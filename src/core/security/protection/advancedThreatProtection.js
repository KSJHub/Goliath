...
function sessionKey(guildId, actorId) { return `${guildId}:${actorId}`; }
function cleanIdArray(value) { return Array.isArray(value) ? [...new Set(value.map((id) => String(id || '').trim()).filter((id) => /^\d{15,25}$/.test(id)))] : []; }
function getProtectedAssets(guild) {
  const saved = guildManager.getGuildSection(guild.id, 'antiNuke', {}) || {};
  const protectedAssets = saved.protectedAssets && typeof saved.protectedAssets === 'object' ? saved.protectedAssets : {};
  ...
  return { enabled: protectedAssets.enabled !== false, roleIds, channelIds, roleIdList: [...roleIds], channelIdList: [...channelIds] };
}
...
function getTargetId(entry) { return String(entry?.targetId || entry?.target?.id || '').trim() || null; }
function protectedTargetBonus(guild, entry, rule) {
  ...
  if (rule.targetType === 'role' && assets.roleIds.has(targetId)) return { bonus: 55, protectedAsset: true, protectedAssetType: 'role' };
  if (rule.targetType === 'channel' && assets.channelIds.has(targetId)) return { bonus: 55, protectedAsset: true, protectedAssetType: 'channel' };
...
async function memberRoleGrantBonus(guild, entry) {
  ...
  const additions = [];
  for (const change of entry.changes || []) if (change?.key === '$add' && Array.isArray(change.new)) for (const value of change.new) if (value?.id) additions.push(String(value.id));
...
function pruneSession(session, now = Date.now()) {
  ...
  return { id: session?.id || `threat_${now}_${Math.random().toString(36).slice(2, 8)}`, startedAt: events[0]?.at || now, updatedAt: events[events.length - 1]?.at || now, events };
}
function recordThreatEvent(guildId, actorId, event) {
  ...
  return { ...current, totalScore, distinctTypes: new Set(current.events.map((item) => item.type)).size };
}
...
function sanitizeChanges(changes = []) { return (Array.isArray(changes) ? changes : []).slice(0, 20).map((change) => ({ key: change?.key || null, old: change?.old ?? null, new: change?.new ?? null })); }

// Never persist a full emergency-backup snapshot inside security history. The
// snapshot contains guildConfig -> security history -> prior snapshots, which
// otherwise grows recursively on every incident and can exhaust process memory.
function compactBackupResult(value) {
  if (!value || typeof value !== 'object') return value || null;
  return {
    success: value.success !== false,
    backupId: value.backupId || value.id || value.backup?.id || null,
    backupType: value.backupType || value.type || null,
    reason: value.reason || null,
    createdAt: value.createdAt || value.timestamp || value.backup?.createdAt || null,
    path: value.path || value.filePath || null,
  };
}
function compactActionResult(value) {
  if (!value || typeof value !== 'object') return value || null;
  const compact = {};
  for (const key of ['success', 'active', 'reason', 'action', 'durationMs', 'expiresAt', 'createdAt', 'restored', 'count', 'message']) {
    if (value[key] !== undefined && (value[key] === null || ['string', 'number', 'boolean'].includes(typeof value[key]))) compact[key] = value[key];
  }
  return Object.keys(compact).length ? compact : { success: value.success !== false };
}
function compactResponse(response = {}) {
  return {
    beforeBackup: compactBackupResult(response.beforeBackup),
    isolation: compactActionResult(response.isolation),
    lockdown: compactActionResult(response.lockdown),
    inviteFreeze: compactActionResult(response.inviteFreeze),
    roleFreeze: compactActionResult(response.roleFreeze),
    ownerAlerted: Boolean(response.ownerAlerted),
    afterBackup: compactBackupResult(response.afterBackup),
  };
}
function persistIncidentPackage(guild, payload) {
  const packageData = { packageId: payload.packageId || `pkg_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`, guildId: guild.id, guildName: guild.name, ...payload, response: compactResponse(payload.response), createdAt: payload.createdAt || new Date().toISOString() };
  guildManager.updateSecurityConfig(guild.id, (current = {}) => ({ ...current, incidentPackages: [packageData, ...(Array.isArray(current.incidentPackages) ? current.incidentPackages : [])].slice(0, 100) }), guild);
  return packageData;
}
async function runResponse({ guild, config, member, severity, reason }) {
  const response = { beforeBackup: null, isolation: null, lockdown: null, inviteFreeze: null, roleFreeze: null, ownerAlerted: false, afterBackup: null };
  ...
  if (high && config.backups?.beforeIncident !== false) response.beforeBackup = await antiNuke.createEmergencyBackup(guild, `Before correlated ${severity} threat response`, 'before_correlated_threat_response');
  if (high && member) response.isolation = await antiNuke.quarantineMember(guild, member, config, reason);
  if (critical) {
    response.lockdown = await antiNuke.emergencyLockdown(guild, reason);
    if (config.emergencyControls?.enabled !== false && config.emergencyControls?.disableInvites !== false) response.inviteFreeze = await disableInvites(guild, { reason, durationMs: config.emergencyControls?.durationMs, trustedRoleIds: config.trustedRoleIds });
    if (config.emergencyControls?.enabled !== false && config.emergencyControls?.freezeRoles !== false) response.roleFreeze = await freezeRoles(guild, { reason, durationMs: config.emergencyControls?.durationMs, trustedRoleIds: config.trustedRoleIds });
  }
  return response;
}
async function handleAuditLogEntry(entry, guild) {
  ...
  if (isTrusted(member, guild, config) || (config.ignoreBots && entry.executor?.bot)) return null;
  ...
  const targetId = getTargetId(entry);
  const now = Date.now();
  const session = recordThreatEvent(guild.id, actorId, { at: now, type: rule.type, action: entry.action, targetId, targetType: rule.targetType, weight: eventWeight, protectedAsset: protectedResult.protectedAsset });
  ...
  const reason = protectedResult.protectedAsset ? `Protected ${protectedResult.protectedAssetType} security event detected: ${rule.type}.` : `Suspicious security event detected: ${rule.type}.`;
  const response = await runResponse({ guild, config, member, severity, reason });
  const persistedResponse = compactResponse(response);
  const packageData = persistIncidentPackage(guild, { sessionId: session.id, actorId, actorTag: entry.executor?.tag || entry.executor?.username || null, severity, threatScore: score, distinctEventTypes: session.distinctTypes, auditEvidence: { auditEntryId: entry.id || null, action: entry.action, reason: entry.reason || null, targetId, targetType: rule.targetType, changes: sanitizeChanges(entry.changes) }, protectedAsset: protectedResult, dangerousRoleGrants: roleGrant.dangerousRoles, sessionEvents: session.events, response: persistedResponse });
  const actionParts = [];
  ...
  const incident = await securitySystem.logIncident(guild, { type: `advanced_${rule.type}`, severity, actorId, actorTag: entry.executor?.tag || entry.executor?.username || null, targetId, targetName: entry.target?.name || entry.target?.username || null, targetType: rule.targetType, reason, actionTaken: actionParts.join('; '), metadata: { threatScore: score, sessionId: session.id, packageId: packageData.packageId, distinctEventTypes: session.distinctTypes, protectedAsset: protectedResult, dangerousRoleGrants: roleGrant.dangerousRoles, response: persistedResponse } });
  if ([securitySystem.SEVERITY.MEDIUM, securitySystem.SEVERITY.HIGH, securitySystem.SEVERITY.CRITICAL].includes(severity)) response.ownerAlerted = await antiNuke.alertOwner(guild, incident);
  if (severity === securitySystem.SEVERITY.CRITICAL && config.backups?.afterIncident !== false) response.afterBackup = await antiNuke.createEmergencyBackup(guild, `After correlated critical threat response: ${rule.type}`, 'after_correlated_threat_response');
...
module.exports = { SESSION_WINDOW_MS, getProtectedAssets, getThreatSession, handleAuditLogEntry, persistIncidentPackage, compactResponse };
