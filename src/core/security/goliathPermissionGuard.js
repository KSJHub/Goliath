...
async function guardChannelAccess(guild, channelId, requiredPermissions = DEFAULT_BOT_CHANNEL_PERMISSIONS, options = {}) {
  ...
  return resolveGuardResult({ guild, targetId: channelId, targetType: 'channel', requiredPermissions, result, options });
}
...
async function guardCategoryAccess(guild, categoryId, requiredPermissions = MANAGE_CHANNEL_PERMISSIONS, options = {}) {
  ...
  return resolveGuardResult({ guild, targetId: categoryId, targetType: 'category', requiredPermissions, result, options });
}
...
async function resolveGuardResult({ guild, targetId, targetType, requiredPermissions, result, options = {} } = {}) {
  ...
  if (canAttemptAutoFix && options.requireConfirmation === true) {
    return buildAutoFixConfirmation(result, { targetId, targetType, requiredPermissions, scope: options.scope });
  }
...
async function applyPermissionAutoFix(guild, targetId, targetType = 'channel', requiredPermissions = DEFAULT_BOT_CHANNEL_PERMISSIONS, options = {}) {
  if (targetType === 'category') return syncBotToCategory(guild, targetId, requiredPermissions, options);
...
function isAutoFixableResult(result = {}) {
  ...
  return failures.every((failure) => failure.reason === 'missing_channel_permission' || failure.reason === 'missing_category_permission');
}
...
function buildAutoFixMessage(result = {}) {
  ...
  const lines = ['⚠️ Goliath needs additional permissions.', '', `Target: ${target}`];
