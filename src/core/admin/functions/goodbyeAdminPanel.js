...
function templateMenu(guildId, activeTemplateId) {
  ...
  const menu = new StringSelectMenuBuilder()
    ...
    .setMinValues(1).setMaxValues(1).setDisabled(templates.length === 0);
  if (templates.length) menu.addOptions(templates.map((template) => ({
...
async function buildGoodbyeAdminPanel(guild, memberDisplayName = 'Unknown User') {
  ...
  return { embeds: [embed], components: [
    row(new ChannelSelectMenuBuilder().setCustomId('admin:goodbye:channel').setPlaceholder('Select the public goodbye channel').setChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement).setMinValues(0).setMaxValues(1)),
    ...
    row(
      button('admin:goodbye:test', '🧪 Test Goodbye', ButtonStyle.Success),
...
async function updatePanel(interaction) {
  const payload = await buildGoodbyeAdminPanel(interaction.guild, interaction.member?.displayName || interaction.user?.username);
  if (interaction.deferred || interaction.replied) return interaction.editReply(payload);
  return interaction.update(payload);
}

async function handleGoodbyeAdminInteraction(interaction) {
...
module.exports = { buildGoodbyeAdminPanel, handleGoodbyeAdminInteraction };