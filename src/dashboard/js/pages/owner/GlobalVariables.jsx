...
export default function GlobalVariables({ theme }) {
  ...
  return <div style={{ display: 'grid', gap: 18 }}>
    ...
    <section style={card}><h2 style={{ marginTop: 0 }}>System Variables <span style={{ color: theme.mutedText, fontSize: 14 }}>({filteredSystem.length})</span></h2><p style={{ color: theme.mutedText }}>Built into Goliath and protected from editing or deletion.</p><div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>{filteredSystem.map((item) => <code key={item.token || item.key} style={{ border: `1px solid ${theme.cardBorder}`, borderRadius: 8, padding: '6px 8px' }}>{item.token || `{${item.key}}`}</code>)}</div></section>
  </div>;
}
