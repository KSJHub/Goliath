...
export default function Goodbye({ theme, selectedGuild, selectedGuildData }) {
  ...
  async function load() {
    ...
    try {
      ...
      setConfig(payload.config || {}); setOverview(payload.overview || {}); setTemplates(payload.templates || []); setChannels(normalizeList(channelPayload, 'channels'));
    } catch (loadError) { setError(loadError.message || 'Failed to load Goodbye.'); } finally { setBusy(''); }
  }
  useEffect(() => { load(); }, [guildId]);
  async function act(name, fn, successText) {
    setBusy(name); setError(''); setNotice('');
    try { const result = await fn(); if (result?.config) setConfig(result.config); if (result?.overview) setOverview(result.overview); if (successText) setNotice(successText); await load(); }
    catch (actionError) { setError(actionError.message || 'Goodbye action failed.'); }
    finally { setBusy(''); }
  }
  ...
  return <div style={{ display: 'grid', gap: 18 }}>
    <section style={{ ...card, background: 'linear-gradient(135deg, rgba(239,68,68,0.16), rgba(15,23,42,0.08) 48%, rgba(59,130,246,0.14))' }}><div style={{ color: '#fca5a5', fontWeight: 950, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Goliath Community</div><h1 style={{ margin: '8px 0 0', fontSize: 'clamp(28px,4vw,42px)' }}>Goodbye</h1><p style={{ color: theme.mutedText, lineHeight: 1.6 }}>Configure public farewell messages, Embed Studio templates, testing, health and delivery analytics.</p></section>
    {(error || notice) ? <section style={{ ...card, color: error ? '#fca5a5' : '#86efac', fontWeight: 850 }}>{error || notice}</section> : null}
    <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,170px),1fr))', gap: 12 }}><Stat theme={theme} label="Status" value={overview.enabled || config?.enabled ? 'Enabled' : 'Disabled'} /><Stat theme={theme} label="Sent" value={analytics.sent || 0} /><Stat theme={theme} label="Failed" value={analytics.failed || 0} /><Stat theme={theme} label="Skipped" value={analytics.skipped || 0} /><Stat theme={theme} label="Health" value={health?.healthy ? 'Healthy' : 'Attention'} hint={`${health?.warnings?.length || 0} warning(s)`} /></section>
    <section style={{ ...card, display: 'grid', gap: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}><div><h2 style={{ margin: 0 }}>Module Setup</h2><p style={{ margin: '6px 0 0', color: theme.mutedText }}>Embed Studio owns the message design; Goodbye owns automatic delivery.</p></div><button type="button" disabled={busy} onClick={() => savePatch({ enabled: !(overview.enabled || config?.enabled) }, overview.enabled || config?.enabled ? 'Goodbye disabled.' : 'Goodbye enabled.')} style={buttonStyle(theme, 'primary')}>{overview.enabled || config?.enabled ? 'Disable' : 'Enable'}</button></div>
      <ChannelSelect theme={theme} resources={channels} value={config?.channelId || ''} onChange={(value) => savePatch({ channelId: value || null })} label="Public Goodbye Channel" />
