...
function isMissingLivePostError(value) {
  ...
  return message.includes('saved live post could not be found') || message.includes('saved live post channel is unavailable');
}

function incidentFor(guild, account) {
  return { guildId: guild?.id || null, guildName: guild?.name || null, module: 'social', component: `${account?.platform || 'unknown'}:${account?.username || account?.externalId || account?.accountId || 'account'}`, code: 'live-message-missing' };
}
...
async function recordRecoveryAction(client, guild, account, currentEventId, input = {}) {
  await audit.captureGoliathAction(client, {
    ...
    type: 'goliath.automation.social_live_post_recovery', category: 'goliath', action: 'execute', system: 'Social Studio',
    result: input.success ? 'Success' : 'Failed',
    summary: input.success
      ? `Social Studio automatically replaced the missing LIVE post for ${account.platform}:${account.username || account.externalId || account.accountId}.`
      : `Social Studio failed to replace the missing LIVE post for ${account.platform}:${account.username || account.externalId || account.accountId}: ${String(input.error || 'Unknown recovery failure').slice(0, 300)}`,
    target: { type: 'social_account', id: String(account.accountId), label: `${account.platform}:${account.username || account.externalId || account.accountId}` },
    reason: input.success ? 'Tracked LIVE message was missing and required automatic replacement.' : String(input.error || 'Automatic LIVE post recovery failed').slice(0, 500),
    metadata: { automatic: true, accountId: account.accountId, platform: account.platform, currentEventId: String(currentEventId), deliveredMessageId: input.delivery?.messageId || null, deliveredChannelId: input.delivery?.channelId || null },
  }).catch((error) => console.warn('[Social Studio] Could not record LIVE recovery audit action:', error?.message || error));
}
...
async function recoverAccount(client, guild, guildConfig, account) {
  ...
  await sentinel.report(client, { ...incident, severity: 'warning', message: 'The tracked Social Studio LIVE post is missing. Automatic replacement started.', details: { accountId: account.accountId, currentEventId: String(currentEventId), missingMessageId: state.lastLiveMessageId || state.lastAlertMessageId || null, missingChannelId: state.lastLiveMessageChannelId || state.lastAlertChannelId || null, deliveryError: state.lastDeliveryError || null } });
  try {
    ...
    await recordRecoveryAction(client, guild, account, currentEventId, { success: true, delivery });
    ...
    await sentinel.report(client, { ...incident, severity: 'error', message: 'Social Studio detected a missing LIVE post but automatic replacement failed.', details: { accountId: account.accountId, currentEventId: String(currentEventId), error: error?.stack || error?.message || String(error) } });
    await recordRecoveryAction(client, guild, account, currentEventId, { success: false, error: error?.message || String(error) });
...
module.exports = { name: 'clientReady', once: true, async execute(client) {
  if (timer) return;
  const initial = setTimeout(() => sweep(client).catch(() => null), 15_000); initial.unref?.();
  timer = setInterval(() => sweep(client).catch(() => null), RECOVERY_INTERVAL_MS); timer.unref?.();
