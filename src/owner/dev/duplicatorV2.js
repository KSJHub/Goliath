...
function duplicatorCreateChannelType(type) { const supported = new Set([ChannelType.GuildText, ChannelType.GuildVoice, ChannelType.GuildCategory, ChannelType.GuildAnnouncement, ChannelType.GuildStageVoice, ChannelType.GuildForum, ChannelType.GuildMedia]); return supported.has(type) ? type : ChannelType.GuildText; }
...
async function applyRoles(guild, snap, maps, log, conflictMode) { const names = new Set(guild.roles.cache.map((r) => r.name.toLowerCase())); const botHighest = guild.members.me?.roles?.highest?.position ?? 0; for (const role of [...(snap.roles || [])].sort((a, b) => a.position - b.position)) { try { const found = existingRole(guild, role.name); if (found && conflictMode === 'skip') { maps.roles.set(role.id, found.id); log.skipped.push(`Role exists: ${role.name}`); continue; } if (found && conflictMode === 'replace') { if (found.editable && found.position < botHighest) { await found.delete('Goliath duplicator: replace role'); log.deleted.roles += 1; } else { maps.roles.set(role.id, found.id); log.skipped.push(`Role not editable due to Discord hierarchy: ${role.name}`); continue; } } const name = found && conflictMode === 'rename' ? uniqueName(names, role.name, 100) : role.name; let created; const requestedPermissions = safeRolePermissions(guild, role.permissions, role.name, log); try { created = await guild.roles.create({ name, colors: { primaryColor: Number(role.color || 0) }, hoist: Boolean(role.hoist), mentionable: Boolean(role.mentionable), permissions: requestedPermissions, reason: 'Goliath duplicator: role' }); } catch (error) { if (Number(error?.code) !== 50013) throw error; created = await guild.roles.create({ name, colors: { primaryColor: Number(role.color || 0) }, hoist: Boolean(role.hoist), mentionable: Boolean(role.mentionable), permissions: new PermissionsBitField(0n), reason: 'Goliath duplicator: role (permission-safe retry)' }); log.notes.push(`Role ${role.name}: created with permissions stripped because Discord rejected source permissions.`); } maps.roles.set(role.id, created.id); names.add(created.name.toLowerCase()); log.copied.roles += 1; } catch (error) { pushError(log, `Role ${role.name}`, error); log.skipped.push(`Role failed: ${role.name}`); } } }
function channelPayload(channel, parentId = null, name = null) { const payload = { name: name || channel.name, type: duplicatorCreateChannelType(channel.type), reason: 'Goliath duplicator: channel' }; if (parentId) payload.parent = parentId; if ([ChannelType.GuildText, ChannelType.GuildAnnouncement, ChannelType.GuildForum, ChannelType.GuildMedia].includes(channel.type)) { payload.topic = channel.topic || undefined; payload.nsfw = channel.nsfw; payload.rateLimitPerUser = channel.rateLimitPerUser || 0; } if ([ChannelType.GuildVoice, ChannelType.GuildStageVoice].includes(channel.type)) { payload.bitrate = channel.bitrate || undefined; payload.userLimit = channel.userLimit || 0; payload.rtcRegion = channel.rtcRegion || undefined; payload.videoQualityMode = channel.videoQualityMode || undefined; } if ([ChannelType.GuildForum, ChannelType.GuildMedia].includes(channel.type)) { payload.defaultAutoArchiveDuration = channel.defaultAutoArchiveDuration || undefined; payload.defaultThreadRateLimitPerUser = channel.defaultThreadRateLimitPerUser || 0; if (channel.availableTags?.length) payload.availableTags = channel.availableTags; } return payload; }
