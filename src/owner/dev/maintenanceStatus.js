...
function isMaintenanceChannel(channel) {
  return Boolean(channel && channel.type === ChannelType.GuildText && (channel.topic === CHANNEL_TOPIC || channel.name === CHANNEL_NAME));
}
...
async function beginGuildMaintenance(guild, options = {}) {
  ...
  if (message) await message.edit({ embeds: [maintenanceEmbed(startedAt, reason)] });
  else message = await channel.send({ embeds: [maintenanceEmbed(startedAt, reason)] });
  await message.pin().catch(() => null);
  return { ok: true, guildId: guild.id, channelId: channel.id, messageId: message.id, startedAt };
}
...
function scheduleChannelDeletion(channel, delayMs = RECOVERY_DELETE_DELAY_MS) {
  ...
  const timer = setTimeout(async () => {
    ...
    await channel.delete('Goliath maintenance completed').catch(() => null);
...
async function recoverGuild(guild, options = {}) {
  ...
  if (options.force !== true && !noticeAllowed(guild.id, 'recovery')) {
    scheduleChannelDeletion(channel, 5000);
    return { ok: true, guildId: guild.id, found: true, skipped: true, reason: 'recovery-notice-disabled', channelId: channel.id };
  }
  const restoredAt = Number(options.restoredAt || Date.now());
  let message = await findStatusMessage(channel);
  const startedAt = message?.createdTimestamp || restoredAt;
  if (message) await message.edit({ embeds: [operationalEmbed(startedAt, restoredAt)] });
  else { message = await channel.send({ embeds: [operationalEmbed(startedAt, restoredAt)] }); await message.pin().catch(() => null); }
...
async function recoverAll(client, options = {}) {
  const results = [];
  for (const guild of client?.guilds?.cache?.values?.() || []) {
    try { results.push(await recoverGuild(guild, options)); }
    catch (error) { results.push({ ok: false, guildId: guild?.id || null, error: error?.message || String(error) }); }
  }
  return results;
}
...
function guildOptions(client) {
  ...
  for (const item of auditStore.getGuildRegistry?.() || []) {
    ...
    merged.set(id, { id, name: item.name || id, environments: Object.keys(item.environments || {}), live: client.guilds.cache.has(id) });
  }
  for (const guild of client.guilds.cache.values()) {
    if (guild.id === destination) continue;
    const current = merged.get(guild.id) || { environments: [] };
    merged.set(guild.id, { ...current, id: guild.id, name: guild.name, live: true, environments: current.environments.length ? current.environments : [String(process.env.BOT_MODE || 'DEV').toUpperCase()] });
  }
  return [...merged.values()].sort((a, b) => a.name.localeCompare(b.name)).slice(0, 25);
}
function session(interaction) {
  const key = `${interaction.guildId}:${interaction.user.id}`;
  if (!controlSessions.has(key)) controlSessions.set(key, { guildId: null, family: 'goliath', scan: null });
  return controlSessions.get(key);
}
function setSession(interaction, patch) { const current = session(interaction); Object.assign(current, patch); return current; }
function reportingSettings(guildId) {
  const cfg = auditStore.getConfig().guilds?.[String(guildId || '')] || {};
  return { enabled: cfg.enabled !== false, monitoring: cfg.monitoring && typeof cfg.monitoring === 'object' ? cfg.monitoring : {} };
}
function updateReporting(guildId, patch = {}) {
  const current = auditStore.getConfig().guilds?.[String(guildId || '')] || {};
  updateGuildConfig(guildId, { ...patch, monitoring: patch.monitoring || current.monitoring || {} });
}
async function scanLiveGuild(guild) {
  if (!guild) return { ok: false, reason: 'remote-collector-required', scannedAt: new Date().toISOString() };
  const started = Date.now();
  const [members, bans, invites, auditLogs, automod] = await Promise.all([
    guild.members.fetch().catch(() => guild.members.cache),
    guild.bans.fetch().catch(() => null),
    guild.invites.fetch().catch(() => null),
    guild.fetchAuditLogs({ limit: 100 }).catch(() => null),
    guild.autoModerationRules?.fetch?.().catch(() => null),
  ]);
  ...
  return {
    ...
    members: members?.size ?? guild.memberCount ?? 0,
    ...
    bans: bans?.size ?? null,
    invites: invites?.size ?? null,
    emojis: emojis.size,
    stickers: stickers.size,
    automod: automod?.size ?? null,
    auditEntries: auditLogs?.entries?.size ?? null,
  };
}
function scanSummary(scan) {
  if (!scan) return 'Not scanned in this Command Center session.';
  if (!scan.ok) return '⚪ This guild is attached to another collector. Remote Observatory scanning will be handled by the cross-environment scanner.';
  return [
    `🟢 **Live scan completed** • ${scan.durationMs}ms`,
    `Members **${scan.members}** • Bots **${scan.bots}** • Roles **${scan.roles}**`,
    `Channels **${scan.channels}** • Categories **${scan.categories}**`,
    `Bans **${scan.bans ?? 'No access'}** • Invites **${scan.invites ?? 'No access'}**`,
    `Emojis **${scan.emojis}** • Stickers **${scan.stickers}** • AutoMod **${scan.automod ?? 'No access'}**`,
    `Recoverable Audit Log entries read **${scan.auditEntries ?? 'No access'}**`,
    `Scanned <t:${unixSeconds(Date.parse(scan.scannedAt))}:R>`,
  ].join('\n');
}
function guildControlPayload(client, interaction, notice = null) {
  const state = session(interaction);
  const options = guildOptions(client);
  if (!state.guildId && options.length) state.guildId = options[0].id;
  const selected = options.find((item) => item.id === state.guildId) || null;
  const notices = selected ? noticeSettings(selected.id) : DEFAULT_NOTICE_SETTINGS;
  const reporting = selected ? reportingSettings(selected.id) : { enabled: true, monitoring: {} };
  const liveGuild = selected ? client.guilds.cache.get(selected.id) : null;
  const statusChannel = liveGuild ? findMaintenanceChannel(liveGuild) : null;
  const familyEnabled = reporting.monitoring[state.family] !== false;
  const reportLines = Object.entries(REPORT_FAMILIES).map(([key, label]) => `${reporting.monitoring[key] === false ? '🔴' : '🟢'} ${label}`).join('\n');
  const embed = new EmbedBuilder().setColor(0x5865F2).setTitle('🎛️ GOLIATH COMMAND CENTER • GUILD CONTROLS')
    ...
    .addFields(
      ...
      { name: 'Collectors', value: selected ? (selected.environments.join(' • ') || 'Registry') : '—', inline: true },
      { name: 'Live Here', value: liveGuild ? '🟢 Yes' : '⚪ Remote collector', inline: true },
      { name: 'Sentinel Collection', value: '🔒 **ALWAYS ON — EVERYTHING**', inline: false },
      { name: 'Observatory Scan', value: scanSummary(state.scan).slice(0, 1024), inline: false },
      { name: 'Operational Notices', value: selected ? `${notices.paused ? '🔕 PAUSED' : '🔔 ACTIVE'}\n${notices.maintenance ? '🟢' : '🔴'} Maintenance • ${notices.restart ? '🟢' : '🔴'} Restart/Offline • ${notices.recovery ? '🟢' : '🔴'} Recovery\nStatus channel: ${statusChannel ? `<#${statusChannel.id}>` : 'None'}` : 'Select a guild.', inline: false },
  ...
  if (selected) {
    rows.push(new ActionRowBuilder().addComponents(
      ...
      new ButtonBuilder().setCustomId(`${CONTROL_PREFIX}scan`).setLabel('Scan Guild').setEmoji('🔎').setStyle(ButtonStyle.Primary).setDisabled(!liveGuild),
    ));
...
async function handleControlInteraction(client, interaction) {
  ...
  if (!id.startsWith(CONTROL_PREFIX)) return false;
  const config = auditStore.getConfig();
  if (String(interaction.guildId || '') !== String(config.commandCenter?.guildId || '')) return false;
  ...
  const action = id.slice(CONTROL_PREFIX.length);
  const state = session(interaction);
  let notice = null;
  if (action === 'open') {
    await interaction.reply({ ...guildControlPayload(client, interaction), flags: MessageFlags.Ephemeral }).catch(() => null);
    return true;
  }
  if (action === 'guild' && interaction.isStringSelectMenu?.()) setSession(interaction, { guildId: interaction.values[0], scan: null });
    ...
    await interaction.reply({ content: 'Select a guild first.', flags: MessageFlags.Ephemeral }).catch(() => null);
    return true;
  } else if (action === 'pause') {
    const n = noticeSettings(state.guildId); const next = updateNoticeSettings(state.guildId, { paused: !n.paused });
    notice = next.paused ? 'Operational notices paused for this guild.' : 'Operational notices resumed for this guild.';
  } else if (['maintenance', 'restart', 'recovery'].includes(action)) {
    const n = noticeSettings(state.guildId); const next = updateNoticeSettings(state.guildId, { [action]: !n[action] });
    notice = `${action[0].toUpperCase()}${action.slice(1)} notices ${next[action] ? 'enabled' : 'disabled'} for this guild.`;
  } else if (action === 'family-toggle') {
    const r = reportingSettings(state.guildId); updateReporting(state.guildId, { monitoring: { ...r.monitoring, [state.family]: r.monitoring[state.family] === false } });
    notice = `${REPORT_FAMILIES[state.family]} reporting updated.`;
  } else if (action === 'reporting-toggle') {
    const r = reportingSettings(state.guildId); updateReporting(state.guildId, { enabled: !r.enabled });
    notice = `Discord reporting ${r.enabled ? 'paused' : 'resumed'} for this guild. Sentinel collection remains on.`;
  } else if (action === 'all-on') {
    updateReporting(state.guildId, { enabled: true, monitoring: Object.fromEntries(Object.keys(REPORT_FAMILIES).map((key) => [key, true])) });
    notice = 'All Discord report feeds enabled for this guild.';
  } else if (action === 'scan') {
    const guild = client.guilds.cache.get(state.guildId);
    setSession(interaction, { scan: await scanLiveGuild(guild) });
    notice = guild ? 'Live Observatory scan completed.' : 'This guild requires its remote collector for a live scan.';
  } else if (action === 'start') {
    const guild = client.guilds.cache.get(state.guildId);
    if (guild) { await beginGuildMaintenance(guild, { force: true, reason: 'Started manually from Goliath Command Center' }); notice = 'Maintenance notice sent manually.'; }
  } else if (action === 'restore') {
    const guild = client.guilds.cache.get(state.guildId);
    if (guild) { await recoverGuild(guild, { force: true }); notice = 'Recovery/clear action completed.'; }
  }
  if (interaction.isMessageComponent?.()) await interaction.update(guildControlPayload(client, interaction, notice)).catch(async () => {
    await interaction.reply({ ...guildControlPayload(client, interaction, notice), flags: MessageFlags.Ephemeral }).catch(() => null);
  });
  return true;
}
async function ensureCommandCenterControls(client) {
  ...
  if (!channel?.isTextBased?.()) channel = guild.channels.cache.find((item) => item.isTextBased?.() && item.name === 'command-center') || null;
  if (!channel) return false;
  const messages = await channel.messages.fetch({ limit: 20 }).catch(() => null);
  if (!messages) return false;
  const message = messages.find((item) => item.author?.id === client.user?.id && item.embeds?.some((embed) => String(embed.title || '').includes('GOLIATH COMMAND CENTER')));
  if (!message) return false;
  const rows = message.components.map((row) => ActionRowBuilder.from(row));
  const exists = message.components.some((row) => row.components.some((component) => component.customId === `${CONTROL_PREFIX}open`));
  if (exists) return true;
  const button = new ButtonBuilder().setCustomId(`${CONTROL_PREFIX}open`).setLabel('Guild Controls').setEmoji('🎛️').setStyle(ButtonStyle.Primary);
  const last = rows[rows.length - 1];
  if (last && last.components.length < 5) last.addComponents(button);
...
function wireCommandCenterControls(client) {
  if (commandCenterWired) return false;
  commandCenterWired = true;
  client.on('interactionCreate', (interaction) => {
    handleControlInteraction(client, interaction).catch((error) => console.warn('[CommandCenter Guild Controls]', error?.stack || error?.message || error));
    const id = String(interaction.customId || '');
    if (id.startsWith('owner:commandcenter:') && !id.startsWith(CONTROL_PREFIX)) setTimeout(() => ensureCommandCenterControls(client).catch(() => null), 750).unref?.();
  });
  return true;
}
function wireProcessHandlers(client, options = {}) {
  ...
  const shutdown = async (signal) => {
    ...
    console.log(`[MaintenanceStatus] ${signal} received. Publishing permitted maintenance notices before shutdown.`);
    const forceTimer = setTimeout(() => process.exit(0), exitAfterMs); forceTimer.unref?.();
    try { await beginMaintenanceForAll(client, { kind: 'restart', reason: signal === 'SIGTERM' ? 'Goliath service restart / maintenance' : 'Goliath maintenance shutdown' }); }
    catch (error) { console.error('[MaintenanceStatus] Shutdown notice failed:', error?.stack || error?.message || error); }
    try { if (server?.listening) await new Promise((resolve) => server.close(() => resolve())); } catch {}
