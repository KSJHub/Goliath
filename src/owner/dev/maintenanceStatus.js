...
async function findStatusMessage(channel) {
  ...
  const pinnedMessage = pinned?.find?.((message) => message.author?.id === channel.client.user?.id); if (pinnedMessage) return pinnedMessage;
...
async function beginGuildMaintenance(guild, options = {}) {
  ...
  const startedAt = Number(options.startedAt || Date.now()); const reason = options.reason || 'Scheduled maintenance';
  const channel = await ensureMaintenanceChannel(guild); let message = await findStatusMessage(channel);
  if (message) await message.edit({ embeds: [maintenanceEmbed(startedAt, reason)] }); else message = await channel.send({ embeds: [maintenanceEmbed(startedAt, reason)] });
...
async function deleteGuildMaintenanceChannel(guild) {
  ...
  await channel.delete('Goliath maintenance channel manually cleared'); return { ok: true, found: true, guildId: guild.id, channelId: channel.id };
}
...
function guildOptions(client) {
  ...
  for (const item of auditStore.getGuildRegistry?.() || []) {
    const id = String(item.guildId || ''); if (!id || id === destination) continue;
...
function session(interaction) {
  ...
  if (!controlSessions.has(key)) controlSessions.set(key, { guildId: null, family: 'goliath' });
...
function guildControlPayload(client, interaction, notice = null) {
  const state = session(interaction); const options = guildOptions(client);
...
async function handleControlInteraction(client, interaction) {
  ...
  const action = id.slice(CONTROL_PREFIX.length); const state = session(interaction);
  ...
  if (action === 'guild' && interaction.isStringSelectMenu?.()) setSession(interaction, { guildId: interaction.values[0] });
  ...
  else if (action === 'pause') { const n = noticeSettings(state.guildId); updateNoticeSettings(state.guildId, { paused: !n.paused }); }
  else if (['maintenance', 'restart', 'recovery'].includes(action)) { const n = noticeSettings(state.guildId); updateNoticeSettings(state.guildId, { [action]: !n[action] }); }
  else if (action === 'family-toggle') { const r = reportingSettings(state.guildId); updateReporting(state.guildId, { monitoring: { ...r.monitoring, [state.family]: r.monitoring[state.family] === false } }); }
  else if (action === 'reporting-toggle') { const r = reportingSettings(state.guildId); updateReporting(state.guildId, { enabled: !r.enabled }); }
  else if (action === 'all-on') { const r = reportingSettings(state.guildId); updateReporting(state.guildId, { enabled: true, monitoring: Object.fromEntries(Object.keys(REPORT_FAMILIES).map((key) => [key, true])) }); }
  else if (action === 'start') { const guild = client.guilds.cache.get(state.guildId); if (guild) await beginGuildMaintenance(guild, { force: true, reason: 'Started manually from Goliath Command Center' }); }
  else if (action === 'restore') { const guild = client.guilds.cache.get(state.guildId); if (guild) await recoverGuild(guild, { force: true }); }
  if (interaction.isMessageComponent?.()) await interaction.update(guildControlPayload(client, interaction)).catch(async () => { await interaction.reply({ ...guildControlPayload(client, interaction), flags: MessageFlags.Ephemeral }).catch(() => null); });
  return true;
}
async function ensureCommandCenterControls(client) {
  ...
  const exists = message.components.some((row) => row.components.some((component) => component.customId === `${CONTROL_PREFIX}open`)); if (exists) return true;
...
function wireProcessHandlers(client, options = {}) {
  ...
  const shutdown = async (signal) => {
    if (shutdownInProgress) return; shutdownInProgress = true;
