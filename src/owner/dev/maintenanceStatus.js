...
function maintenanceStartedAt(message, fallback = Date.now()) {
  ...
  const footerMatch = String(footer).match(/Started\s+(\d{9,13})/i);
  if (footerMatch) {
    const raw = Number(footerMatch[1]); const ms = raw < 1e12 ? raw * 1000 : raw;
  ...
  if (fieldMatch) { const raw = Number(fieldMatch[1]); const ms = raw < 1e12 ? raw * 1000 : raw; if (Number.isFinite(ms) && ms > 0) return ms; }
...
function guildControlPayload(client, interaction, notice = null) {
  ...
  const embed = new EmbedBuilder().setColor(0x5865F2).setTitle('🎛️ GOLIATH COMMAND CENTER • GUILD CONTROLS')
    ...
    .addFields(
      ...
      { name: 'Observatory', value: scanSummary(latest).slice(0, 1024) },
      { name: 'Discord Reporting', value: `${reporting.enabled ? '🟢 Enabled' : '🔴 Paused'}\n${reportLines}`.slice(0, 1024), inline: true },
      { name: 'Operational Notices', value: `${notices.paused ? '🔴 Paused' : '🟢 Active'}\nMaintenance ${notices.maintenance ? 'ON' : 'OFF'} • Restart ${notices.restart ? 'ON' : 'OFF'} • Recovery ${notices.recovery ? 'ON' : 'OFF'}\nStatus channel: ${statusChannel ? `🟠 ${statusChannel}` : 'None'}`.slice(0, 1024), inline: true },
    ).setFooter({ text: 'Goliath Command Center • Owner only • Collection cannot be disabled' }).setTimestamp();
  const rows = [];
  if (options.length) rows.push(new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId(`${CONTROL_PREFIX}guild`).setPlaceholder('Select guild').addOptions(options.map((item) => ({ label: item.name.slice(0, 100), description: `${collectorFor(item)} • ${item.live ? 'Live on this process' : 'Remote collector'}`.slice(0, 100), value: item.id, default: item.id === state.guildId }))));
  if (selected) {
    rows.push(new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`${CONTROL_PREFIX}scan`).setLabel('Scan Guild').setEmoji('🔎').setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId(`${CONTROL_PREFIX}pause`).setLabel(notices.paused ? 'Resume Notices' : 'Pause Notices').setStyle(notices.paused ? ButtonStyle.Success : ButtonStyle.Danger),
    ...
    rows.push(new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId(`${CONTROL_PREFIX}family`).setPlaceholder('Select reporting family').addOptions(Object.entries(REPORT_FAMILIES).map(([value, label]) => ({ label, value, default: value === state.family }))));
    rows.push(new ActionRowBuilder().addComponents(
