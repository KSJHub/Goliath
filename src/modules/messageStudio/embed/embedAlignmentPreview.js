...
function safeUrl(value) {
  try {
    ...
    if (url.protocol !== 'https:' || !host || host === 'localhost' || host.endsWith('.localhost')) return null;
    if ((version === 4 && isPrivateIpv4(host)) || (version === 6 && isPrivateIpv6(host))) return null;
    return url.toString();
  } catch { return null; }
}
...
function alignmentFor(state, panelIndex, itemIndex, item) {
  const key = `${panelIndex}:${itemIndex}`;
  const value = String(state?.mediaAlignment?.[key] || item?.alignment || 'left').toLowerCase();
...
async function previewAttachment(source, alignment) {
  ...
  const trimmed = await sharp(input, { failOn: 'warning' })
    .ensureAlpha()
  ...
  return new AttachmentBuilder(output, { name: 'embed-alignment-preview.png' });
}
function embedTitle(embed) {
  return String(embed?.data?.title || embed?.title || '');
}
async function alignedManagerPayload(panel, interaction) {
  const payload = panel.buildMediaManagerPanel(interaction, panel.memberName(interaction));
  ...
  const item = media?.gallery?.[itemIndex];
  if (!item?.source || item?.type === 'video') return payload;
  let source = String(item.source || '').trim();
  try { source = panel.replaceVars(source, interaction); } catch {}
  if (!safeUrl(source)) return payload;
  const alignment = alignmentFor(state, panelIndex, itemIndex, item);
  try {
    const attachment = await previewAttachment(source, alignment);
    if (!attachment) return payload;
    const preview = Array.isArray(payload?.embeds)
      ? payload.embeds.find((embed) => embedTitle(embed).includes('Selected Media Preview'))
      : null;
    if (!preview || typeof preview.setImage !== 'function') return payload;
    preview.setImage('attachment://embed-alignment-preview.png');
    preview.setTitle(`🖼️ Selected Media Preview • ${item.placement === 'above' ? 'Above Content' : 'Below Content'} • ${alignment === 'center' ? 'Centre' : alignment[0].toUpperCase() + alignment.slice(1)}`);
    payload.files = [...(Array.isArray(payload.files) ? payload.files : []), attachment];
    return payload;
  } catch (error) {
    console.warn('[Embed Preview] Alignment preview failed:', error?.message || error);
    return payload;
  }
}
function installAlignmentPreview(panel, interactions) {
  ...
  interactions.handleInteraction = async (interaction) => {
    ...
    if (customId === 'embed:media-options-back' || customId === 'embed:edit-images') {
      await interaction.update(await alignedManagerPayload(panel, interaction));
      return true;
    }
    if (customId === 'embed:media-gallery-select' && interaction.isStringSelectMenu?.()) {
      const state = panel.getSession(interaction);
      panel.saveSession(interaction, { ...state, selectedMediaIndex: Math.max(0, Number(interaction.values?.[0]) || 0) });
      await interaction.update(await alignedManagerPayload(panel, interaction));
      return true;
    }
