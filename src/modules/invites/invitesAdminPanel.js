...
const DEFAULT_PUBLIC_PANEL = Object.freeze({
...
const DEFAULT_MEMBER_DM = Object.freeze({
...
function updateNestedSettings(guildId, key, patch, meta) {
  ...
  return invites.updateSettings(guildId, {
    [key]: { ...section.settings[key], ...patch },
  }, meta);
}

function restoreDefaultMessages(guildId, meta) {
  const section = invites.getSection(guildId);
  invites.updateSettings(guildId, {
    ...
    memberInviteTemplate: {
      ...section.settings.memberInviteTemplate,
      ...DEFAULT_MEMBER_DM,
    },
  }, meta);

  const saved = invites.getSection(guildId).settings;
  const publicOk = Object.entries(DEFAULT_PUBLIC_PANEL)
    .every(([key, value]) => saved.publicPanel[key] === value);
  const dmOk = Object.entries(DEFAULT_MEMBER_DM)
    .every(([key, value]) => saved.memberInviteTemplate[key] === value);

  if (!publicOk || !dmOk) {
    throw new Error('Invite Studio failed to save the default panel or member DM values.');
  }
  return saved;
}
  ...
  return {
    ...
    components: [
      ...
      row(
        ...
        button(
          resetArmed ? 'invites:leaderboard-reset-confirm' : 'invites:leaderboard-reset-arm',
          resetArmed ? 'Confirm Reset Leaderboard' : 'Reset Leaderboard',
          ButtonStyle.Danger,
        ),
...
async function handleInviteStudioInteraction(interaction) {
  ...
  if (action === 'home') state.page = 'overview';
    ...
    state.resetConfirmUntil = 0;
    await publicPanels.refreshPublicPanel(interaction.guild, {
      actorId: interaction.user.id,
      action: 'invite_admin_reset_refresh',
    }).catch(() => null);
    ...
    const saved = restoreDefaultMessages(interaction.guildId, {
      actorId: interaction.user.id,
      action: 'invite_admin_use_default_panel',
    });
    await publicPanels.refreshPublicPanel(interaction.guild, {
      actorId: interaction.user.id,
      action: 'invite_admin_default_panel_refresh',
    }).catch(() => null);
    await interaction.reply({
      content: [
        '✅ Default panel and member DM restored and verified.',
        `Panel: **${saved.publicPanel.title}**`,
        `DM: **${saved.memberInviteTemplate.dmTitle}**`,
        'Channel, deployed message, leaderboard size, invite links and scores were kept.',
      ].join('\n'),
      flags: MessageFlags.Ephemeral,
    });
    return true;
  } else if (action === 'official-channel' && interaction.isChannelSelectMenu()) {
    updateNestedSettings(interaction.guildId, 'officialInvite', { channelId: interaction.values[0] }, meta);
  } else if (action === 'official-roles' && interaction.isRoleSelectMenu()) {
    updateNestedSettings(interaction.guildId, 'officialInvite', { roleIds: interaction.values }, meta);
  } else if (action === 'official-expiry' && interaction.isStringSelectMenu()) {
    updateNestedSettings(interaction.guildId, 'officialInvite', { maxAge: Number(interaction.values[0]) }, meta);
  } else if (action === 'official-uses' && interaction.isStringSelectMenu()) {
    updateNestedSettings(interaction.guildId, 'officialInvite', { maxUses: Number(interaction.values[0]) }, meta);
    ...
    return true;
  } else if (action === 'member-channel' && interaction.isChannelSelectMenu()) {
    updateNestedSettings(interaction.guildId, 'memberInviteTemplate', { channelId: interaction.values[0] }, meta);
  } else if (action === 'member-roles' && interaction.isRoleSelectMenu()) {
    updateNestedSettings(interaction.guildId, 'memberInviteTemplate', { roleIds: interaction.values }, meta);
  } else if (action === 'member-expiry' && interaction.isStringSelectMenu()) {
    updateNestedSettings(interaction.guildId, 'memberInviteTemplate', { maxAge: Number(interaction.values[0]) }, meta);
  } else if (action === 'member-uses' && interaction.isStringSelectMenu()) {
    updateNestedSettings(interaction.guildId, 'memberInviteTemplate', { maxUses: Number(interaction.values[0]) }, meta);
    ...
    return true;
  } else if (action === 'panel-channel' && interaction.isChannelSelectMenu()) {
    updateNestedSettings(interaction.guildId, 'publicPanel', { channelId: interaction.values[0] }, meta);
  } else if (action === 'panel-limit' && interaction.isStringSelectMenu()) {
    updateNestedSettings(interaction.guildId, 'publicPanel', { leaderboardLimit: Number(interaction.values[0]) }, meta);
