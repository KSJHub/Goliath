...
async function handleAdmin(interaction) {
  ...
  else if (id === 'admin:birthdays:card:defaults') { birthdays.updateSettings(interaction.guildId, { useBirthdayEmbed: true, cardTitle: '🎂 Happy Birthday!', cardFooter: 'From everyone at {server} 🎉', cardColor: '#5865F2', cardImageMode: 'default', cardImageUrl: null, cardUseServerIcon: true }, { ...actor, action: 'birthdays_card_defaults' }); await respond(interaction, cardPayload(interaction)); return true; }
  ...
  else if (id === 'admin:birthdays:card:text:submit') { birthdays.updateSettings(interaction.guildId, { cardTitle: interaction.fields.getTextInputValue('title'), cardFooter: interaction.fields.getTextInputValue('footer') }, { ...actor, action: 'birthdays_card_text_update' }); await interaction.reply({ content: '✅ Birthday Card text updated.', flags: 64 }); return true; }
