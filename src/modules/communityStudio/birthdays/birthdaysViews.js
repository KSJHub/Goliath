...
function cardTextModal(section) { return new ModalBuilder().setCustomId('admin:birthdays:card:text:submit').setTitle('Birthday Card Text').addComponents(row(new TextInputBuilder().setCustomId('title').setLabel('Card title').setStyle(TextInputStyle.Short).setRequired(true).setMaxLength(256).setValue(section.settings.cardTitle)), row(new TextInputBuilder().setCustomId('footer').setLabel('Footer — {server} supported').setStyle(TextInputStyle.Short).setRequired(false).setMaxLength(1000).setValue(section.settings.cardFooter || ''))); }
