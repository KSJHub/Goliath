...
function navigation(backId, settingsId = 'admin:stats:settings', settingsDisabled = false, backLabel = '⬅️ Back') {
  return row(
    button(backId, backLabel, ButtonStyle.Secondary),
    button(settingsId, '⚙️ Settings', ButtonStyle.Secondary, settingsDisabled)
  );
}
...
const generatedTemplate = (guild, segments) => segments
...
async function cleanupFalseAdoptions(guild) {
  ...
  for (const item of listDocks(guild.id)) {
    ...
    if (!looksRenderedCounter(channel, spec, statsCategory)) stats.counters.removeCounter(guild.id, item.id, guild);
  }
}
...
function summaryText(all) {
  ...
  return all.slice(0, 12).map((item) => (
    `${item.enabled ? '🟢' : '⚫'} ${item.channelType === 'text' ? '#️⃣' : '🔊'} **${item.name || 'Counter'}**${item.channelId ? ` — <#${item.channelId}>` : ''}`
  )).join('\n');
}
...
function healthState(guild) {
  const all = listDocks(guild.id);
  const missingChannels = all.filter((item) => item.enabled && item.channelId && !guild.channels.cache.has(item.channelId)).length;
  const unsaved = unsavedCounters(guild).length;
  if (missingChannels) return { color: WARNING_COLOR, text: `⚠️ ${missingChannels} counter${missingChannels === 1 ? '' : 's'} need attention` };
...
function buildMainPanel(guild, requester = 'Management') {
  ...
  const embed = footer(new EmbedBuilder()
    ...
    .setDescription([
      'Create live server statistics that update automatically.',
      '',
      ...
      `**Health:** ${health.text}`,
      '',
      'Use **Setup** to create counters and choose monitored roles. Use **Manage** to edit existing counters or make bulk changes.',
    ].join('\n')), requester);
  ...
  return {
    ...
    components: [
      row(
        ...
        button('admin:stats:manager', '📋 Manage', ButtonStyle.Primary, all.length === 0 && unsavedCounters(guild).length === 0)
      ),
...
function buildSetupPanel(guild, requester = 'Management', notice = '') {
  ...
  const embed = footer(new EmbedBuilder()
    ...
    .setDescription([
      notice ? `> ${notice}` : null,
      notice ? '' : null,
      'Create counters and choose the server roles Goliath should monitor.',
      '',
      `**Saved Counters:** \`${all.length}\``,
      `**Monitored Roles:** \`${roleIds.length}\``,
      '',
      '**Quick Setup** creates Date, Members, Online and In Voice counters. Or choose a counter type below to build one yourself.',
  ...
  return {
    ...
    components: [
      ...
      row(roleSelect('admin:stats:setup:roles', 'Select roles Goliath should monitor…', roleIds)),
...
function buildManagerPanel(guild, requester = 'Management', notice = null) {
  ...
  const embed = footer(new EmbedBuilder()
    ...
    .setDescription(all.length
      ? 'Choose a counter to edit it, add values, change text/voice output, disable it or delete it. Use **Bulk Manage** for multiple counters.'
      : 'No counters have been created yet. Go back and choose **Setup** to create your first counter.')
    .addFields({ name: 'Saved Counters', value: summaryText(all).slice(0, 1024) }), requester);
...
function buildSettingsPanel(guild, requester = 'Management', notice = '') {
  ...
  const embed = footer(new EmbedBuilder()
    ...
    .setDescription([
      ...
      `**Monitored Roles:** \`${monitoredRoleIds(guild.id).length}\``,
      `**Time Zone:** ${settings.timeZone || 'Europe/London'}`,
      `**Refresh:** ${settings.defaultFrequencyMinutes || 10} minutes`,
      '',
      'Category naming, module state and whole-counter maintenance live here.',
    ].filter((line) => line !== null).join('\n')), requester);
  ...
  return {
    ...
    components: [
      row(
        ...
        button('admin:stats:refresh', '🔄 Refresh All', ButtonStyle.Secondary, !all.length)
      ),
      row(
        button('admin:stats:category:rename', '✏️ Category Name & Emoji', ButtonStyle.Secondary),
      ...
      row(button('admin:stats', '⬅️ Back to Server Counters', ButtonStyle.Secondary)),
    ],
  };
}
...
function buildEditPanel(guild, item, requester = 'Management') {
  ...
  return {
    embeds: [footer(new EmbedBuilder()
      ...
      .setDescription('Choose the Discord channel type, then edit each value. One counter channel can display up to **four live values**.')
      .addFields({ name: 'Current Values', value: counterValueLines(guild, item) }), requester)],
    components: [
...
function buildValuePanel(guild, item, index, requester = 'Management') {
  ...
  if (segment.type === 'status') {
    ...
    components.push(row(select(`admin:stats:roletypes:${item.id}:${index}`, 'Choose role types…', optionDefaults([
      { label: 'Server Roles', value: 'unmanaged' },
      { label: 'Integration / Bot Roles', value: 'managed' },
  ...
  components.push(row(
    ...
    button(`admin:stats:edit:${item.id}`, '⬅️ Back', ButtonStyle.Secondary)
  ));
  ...
  return {
    embeds: [footer(new EmbedBuilder()
      ...
      .setDescription(segment.type === 'role'
        ? 'Select the role or roles this value should monitor. Add another value if you want separate totals for separate roles.'
        : 'Change what this value counts.'), requester)],
...
function buildAddPanel(item, requester = 'Management') {
  return {
    embeds: [footer(new EmbedBuilder()
      ...
      .setTitle('➕ Add Counter Value')
      .setDescription(`This counter currently displays **${item.segments.length} / 4** values. Choose another value to add.`), requester)],
    components: [
      row(select(`admin:stats:addtype:${item.id}`, 'Choose a value to add…', typeOptions())),
      row(button(`admin:stats:open:${item.id}`, '⬅️ Cancel', ButtonStyle.Secondary)),
    ],
  };
}
...
function buildDeletePanel(item, requester = 'Management') {
  return {
    embeds: [footer(new EmbedBuilder()
      ...
      .setDescription(`This permanently removes **${item.name || 'Counter'}** and its Discord channel.\n\n**This cannot be undone.**`), requester)],
    components: [row(
      button(`admin:stats:delete-confirm:${item.id}`, '🗑️ Delete Counter', ButtonStyle.Danger),
      button(`admin:stats:open:${item.id}`, 'Cancel', ButtonStyle.Secondary)
    )],
  };
}
...
function buildCategoryDeletePanel(requester = 'Management') {
  return {
    embeds: [footer(new EmbedBuilder()
      .setColor(DANGER_COLOR)
      .setTitle('🗑️ Delete Stats Category?')
      .setDescription('This deletes **all saved counters and their Discord channels**, then removes the Stats category.\n\n**This cannot be undone.**'), requester)],
    components: [row(
      button('admin:stats:category:delete-confirm', '🗑️ Delete Everything', ButtonStyle.Danger),
      button('admin:stats:settings', 'Cancel', ButtonStyle.Secondary)
    )],
  };
}
...
function buildBulkPanel(interaction, requester = 'Management', notice = null) {
  ...
  components.push(row(button('admin:stats:manager', '⬅️ Back to Manage', ButtonStyle.Secondary)));

  const embed = footer(new EmbedBuilder()
    .setColor(notice?.error ? DANGER_COLOR : PANEL_COLOR)
    .setTitle('🧰 Server Counters · Bulk Manage')
    .setDescription([
      `Select up to **${BULK_LIMIT} counters** and apply one action to all of them.`,
      '',
      `**Selected:** \`${selectedItems.length}\``,
      all.length > BULK_LIMIT ? `Showing the first ${BULK_LIMIT} of ${all.length} counters.` : null,
    ].filter(Boolean).join('\n')), requester);
  if (selectedItems.length) embed.addFields({
    name: 'Selected Counters',
    value: selectedItems.slice(0, 12).map((item) => `• ${item.name || 'Counter'}${item.channelId ? ` — <#${item.channelId}>` : ''}`).join('\n').slice(0, 1024),
  });
  if (notice?.text) embed.addFields({ name: notice.error ? '⚠️ Problem' : '✅ Done', value: notice.text.slice(0, 1024) });
  return { embeds: [embed], components };
}
  ...
  return {
    embeds: [footer(new EmbedBuilder()
      ...
      .setDescription([
        ...
        selectedItems.length > 10 ? `• …and ${selectedItems.length - 10} more` : null,
        '',
        '**This cannot be undone.**',
      ].filter(Boolean).join('\n')), requester)],
    components: [row(
      button('admin:stats:bulk:delete-confirm', `🗑️ Delete ${selectedItems.length}`, ButtonStyle.Danger, !selectedItems.length),
      button('admin:stats:bulk', 'Cancel', ButtonStyle.Secondary)
    )],
  };
}
...
function buildErrorPanel(title, message, requester = 'Management', backId = 'admin:stats:manager') {
  return {
    embeds: [footer(new EmbedBuilder()
      .setColor(DANGER_COLOR)
      .setTitle(`⚠️ ${title}`)
      .setDescription(String(message || 'Something went wrong.').slice(0, 4000)), requester)],
...
async function deleteOneCounter(guild, item) {
  ...
  if (item.channelId) {
    const channel = guild.channels.cache.get(item.channelId) || await guild.channels.fetch(item.channelId).catch(() => null);
    if (channel) {
      if (!channel.deletable) throw new Error(`Goliath cannot manage #${channel.name}. Check the bot role and Manage Channels permission.`);
...
async function handleStatsAdminInteraction(interaction) {
  ...
  const guild = interaction.guild;
  const requester = displayName(interaction);
  ...
  if (interaction.isModalSubmit?.() && id === 'admin:stats:category:rename-submit') {
    ...
    return send(interaction, buildSettingsPanel(guild, requester, 'Category name updated.'));
  }
  ...
  if (id === 'admin:stats:setup-page') {
    await interaction.deferUpdate().catch(() => null);
    await reconcile(guild);
    return send(interaction, buildSetupPanel(guild, requester));
  }
  if (id === 'admin:stats:manager') {
    await interaction.deferUpdate().catch(() => null);
    await reconcile(guild);
    return send(interaction, buildManagerPanel(guild, requester));
  }
  if (id === 'admin:stats:settings' || id === 'admin:stats:category') {
    await interaction.deferUpdate().catch(() => null);
    await reconcile(guild);
    return send(interaction, buildSettingsPanel(guild, requester));
  }
  if (id === 'admin:stats:bulk') {
    await interaction.deferUpdate().catch(() => null);
    await reconcile(guild);
    return send(interaction, buildBulkPanel(interaction, requester));
  }
  ...
  if (interaction.isStringSelectMenu?.()) {
    if (id === 'admin:stats:create') {
      ...
      const item = await stats.counters.createDock(guild, {
        name: generatedName(segments),
        template: generatedTemplate(guild, segments),
        channelType: 'voice',
        segments,
        source: 'panel',
      }, guild);
    ...
    if (id === 'admin:stats:bulk:preset') {
      ...
      return send(interaction, buildBulkPanel(interaction, requester, notice));
    }
  ...
  if (id === 'admin:stats:refresh' || id === 'admin:stats:category:refresh') {
    ...
    return send(interaction, buildSettingsPanel(guild, requester, 'All counters refreshed.'));
  }
  ...
  if (id === 'admin:stats:category:toggleall') {
    ...
    return send(interaction, buildSettingsPanel(guild, requester, enable ? 'All counters turned on.' : 'All counters turned off.'));
  }
  ...
  if (id === 'admin:stats:category:delete-confirm') {
    ...
    try {
      ...
      return send(interaction, buildSettingsPanel(guild, requester, 'Stats category and counters deleted.'));
    } catch (error) {
      return send(interaction, buildErrorPanel('Could not delete everything', error.message, requester, 'admin:stats:settings'));
    }
  }
  ...
  if (id === 'admin:stats:bulk:delete-confirm') {
    ...
    for (const item of items) {
      try {
        ...
        failures.push({ id: item.id, name: item.name || 'Counter' });
      }
    }
    setBulkSelection(interaction, failures.map((failure) => failure.id));
    const text = failures.length
      ? `Deleted **${deleted}**. **${failures.length}** could not be deleted because Goliath cannot manage those channels.\n\n${failures.slice(0, 10).map((failure) => `• ${failure.name}`).join('\n')}${failures.length > 10 ? `\n• …and ${failures.length - 10} more` : ''}`
  ...
  if (editMatch) {
    ...
    return send(interaction, item ? buildEditPanel(guild, item, requester) : buildManagerPanel(guild, requester));
  }
  const addMatch = id.match(/^admin:stats:add:(.+)$/);
  if (addMatch) {
    const item = findDock(guild.id, addMatch[1]);
    return send(interaction, item && item.segments.length < 4 ? buildAddPanel(item, requester) : item ? buildManagePanel(guild, item, requester) : buildManagerPanel(guild, requester));
  }
  ...
  if (toggleMatch) {
    ...
    return send(interaction, buildManagePanel(guild, updated, requester));
  }
  ...
  if (deleteConfirmMatch) {
    ...
    try {
      ...
      return send(interaction, buildManagerPanel(guild, requester, { text: `${item.name || 'Counter'} was deleted.` }));
    } catch (error) {
      return send(interaction, buildErrorPanel('Counter could not be deleted', error.message, requester, `admin:stats:open:${item.id}`));
    }
  }
  const deleteMatch = id.match(/^admin:stats:delete:(.+)$/);
  if (deleteMatch) {
    const item = findDock(guild.id, deleteMatch[1]);
    return send(interaction, item ? buildDeletePanel(item, requester) : buildManagerPanel(guild, requester));
  }
