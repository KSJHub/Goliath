...
function creatorEditModal(creator) {
  return new ModalBuilder()
    ...
    .addComponents(
      row(new TextInputBuilder().setCustomId('displayName').setLabel('Creator display name').setPlaceholder('Enter the public creator name here').setStyle(TextInputStyle.Short).setMaxLength(120).setRequired(true).setValue(String(creator.displayName || '').slice(0, 120))),
      row(new TextInputBuilder().setCustomId('group').setLabel('Group or team').setPlaceholder('Add their team, brand or category here').setStyle(TextInputStyle.Short).setMaxLength(120).setRequired(false).setValue(String(creator.group || '').slice(0, 120))),
      ...
      row(new TextInputBuilder().setCustomId('adminNotes').setLabel('Admin Notes (Management Only)').setPlaceholder('Private notes visible only to Social Studio managers.').setStyle(TextInputStyle.Paragraph).setMaxLength(1000).setRequired(false).setValue(String(creator.adminNotes || '').slice(0, 1000))),
    );
}
...
function profilePayload(interaction, creator) {
  ...
  const color = platforms.length === 1 ? { twitch: 0x9146FF, youtube: 0xFF0000, tiktok: 0x2F3136, kick: 0x53FC18, facebook: 0x1877F2, instagram: 0xE1306C, x: 0xFFFFFF }[platforms[0]] || 0x5865F2 : (config.enabled ? 0x5865F2 : 0x747F8D);
...
function saveSettings(interaction, config) {
  return store.saveConfig(interaction.guildId, config, { actorId: interaction.user?.id || null, guild: interaction.guild });
}
...
async function handleMonitoringAction(interaction, id) {
  ...
  if (id === `${P}automation:quiet`) {
    ...
    if (interaction.isModalSubmit?.()) {
      ...
      if (!start || !end) throw new Error('Quiet Hours times must be valid 24-hour times, for example 23:00 and 08:00.');
...
async function handleDiagnosticsAction(interaction, id) {
  ...
  if (id === `${P}testing:diagnostics`) {
    ...
    const lines = platforms.length ? platforms.map((platform) => { let info = {}; try { info = providerInfo(platform) || {}; } catch { info = {}; } const alerts = Array.isArray(info.supportedAlertTypes) && info.supportedAlertTypes.length ? info.supportedAlertTypes.join(', ') : 'No alert types reported'; return `**${platform}** — ${alerts}`; }) : ['No linked accounts are available to inspect.'];
    return followUp(interaction, { content: `🩺 **Social Studio Provider Details**\n\n${lines.join('\n').slice(0, 1800)}` });
  }
...
async function handle(interaction) {
  ...
  if (!creator) { sessions.delete(key(interaction)); throw new Error('The selected creator profile no longer exists.'); }
  if (id === `${P}creator:edit`) { await interaction.showModal(creatorEditModal(creator)); return true; }
  if (id === `${P}creator:clear`) {
    const updated = store.updateCreator(interaction.guildId, creatorId, (current) => ({ ...current, group: '', tags: [], notes: '', adminNotes: '' }), { actorId: interaction.user?.id || null, guild: interaction.guild });
    if (interaction.deferred || interaction.replied) await interaction.editReply(profilePayload(interaction, updated)); else await interaction.update(profilePayload(interaction, updated));
    return true;
  }
  const deleted = store.deleteCreator(interaction.guildId, creatorId, { actorId: interaction.user?.id || null, guild: interaction.guild });
