  ...
  const updated = guildManager.updateGuildSection(
    ...
    (latest = {}) => {
      ...
      for (const [duplicateId, survivorId] of duplicateMerges.entries()) {
        ...
        const alertTypes = [...new Set([...(Array.isArray(survivor.alertTypes) ? survivor.alertTypes : []), ...(Array.isArray(duplicate.alertTypes) ? duplicate.alertTypes : [])])];
        accounts[survivorId] = {
          ...survivor,
        ...
        for (const creator of Object.values(creators)) creator.accountIds = [...new Set((creator.accountIds || []).map((id) => id === duplicateId ? survivorId : id))];
      }
...
function resolvedDuplicateIds(config, account, checked, creator) {
  ...
  for (const otherId of creator.accountIds || []) {
    ...
    if (externalId && otherExternalId && externalId === otherExternalId) { duplicateIds.push(otherId); continue; }
    if (username && otherUsername && username === otherUsername) { duplicateIds.push(otherId); continue; }
...
function addHistory(config, event) { config.history = [...(config.history || []), { id: `history_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`, createdAt: now(), ...event }].slice(-1000); }
...
function secondsBetween(start, end) { const a = new Date(start).getTime(); const b = new Date(end).getTime(); return Number.isFinite(a) && Number.isFinite(b) && b >= a ? Math.floor((b - a) / 1000) : null; }
...
function hasTrackedLiveMessage(state = {}) { return Boolean((state.lastLiveMessageId || state.lastAlertMessageId) && (state.lastLiveMessageChannelId || state.lastAlertChannelId)); }

function eventCandidates(account, previous, checked) {
  const events = [];
  const contentItems = Array.isArray(checked.contentItems) && checked.contentItems.length ? checked.contentItems : checked.latestContent ? [checked.latestContent] : [];
  let endedVodId = null;
  if (checked.isLive === true && checked.event) {
    const key = eventKey(checked.event);
    if (previous.isLive !== true || previous.lastAlertKey !== key || !hasTrackedLiveMessage(previous)) events.push(checked.event);
  }
  if (checked.isLive === false && previous.isLive === true && enabledAlert(account, 'ended')) {
    ...
    events.push({ type: 'ended', id: `ended:${previous.liveEventId || prior.id || account.accountId}:${endedAt}`, title: prior.title || `${account.username || account.displayName || 'Creator'} stream ended`, url: prior.url || account.profileUrl || account.url || '', thumbnail: currentVod?.thumbnail || prior.thumbnail || null, category: prior.category || currentVod?.category || null, startedAt, endedAt, durationSeconds: durationToSeconds(currentVod?.durationSeconds ?? currentVod?.duration) || secondsBetween(startedAt, endedAt), vod: currentVod });
  }
...
function humanDuration(seconds) {
  ...
  const hours = Math.floor(value / 3600), minutes = Math.floor((value % 3600) / 60), secs = Math.floor(value % 60);
  return [hours ? `${hours}h` : '', minutes ? `${minutes}m` : '', !hours && secs ? `${secs}s` : ''].filter(Boolean).join(' ');
}
function discordTimestamp(value, style = 'R') { const ms = new Date(value).getTime(); if (!Number.isFinite(ms) || ms < Date.UTC(2020, 0, 1) || ms > Date.now() + 86400000) return ''; return `<t:${Math.floor(ms / 1000)}:${style}>`; }

function buildLiveFields({ account, event, vars, liveStatus, durationText, started, ended }) {
  const fields = []; const platform = String(account?.platform || '').toLowerCase(); const offline = liveStatus === 'OFFLINE';
  ...
  if (offline) { const peak = Number(account?.state?.peakViewers || vars.peakViewers || event?.viewerCount || 0); if (peak > 0) fields.push({ name: '📈 Peak Viewers', value: intText(peak), inline: true }); }
  ...
  if (offline) { if (ended) fields.push({ name: '⚫ Ended', value: ended, inline: true }); }
...
function eventVars(account, event, creator, options = {}) {
  ...
  return { creator: creatorName, username, platform: PLATFORM[account.platform]?.label || account.platform, platformIcon: PLATFORM[account.platform]?.icon || '🔔', type: event.type, title, url, game, category: game, viewers, peakViewers: intText(account.state?.peakViewers || event.peakViewers || event.viewerCount || 0), duration, started: discordTimestamp(event.startedAt), ended: discordTimestamp(event.endedAt), published: discordTimestamp(event.publishedAt), kickUsername: String(account.platform || '').toLowerCase() === 'kick' ? username : '' };
}
...
async function sendAlert(client, guildId, config, account, event, options = {}) {
  ...
  const content = options.suppressMention === true ? '' : mentionFor(account);
  const message = await channel.send({ content: content || null, embeds: [embed] });
...
function livePostInWindow(config, creator, windowMs = 2 * 60 * 60 * 1000) {
  const accountIds = new Set(creator?.accountIds || []); const cutoff = Date.now() - windowMs;
  return [...(config.history || [])].reverse().find((item) => { if (!accountIds.has(item.accountId)) return false; if (!['alert_sent', 'alert_updated'].includes(item.status) || item.alertType !== 'live') return false; const created = new Date(item.createdAt || 0).getTime(); return Number.isFinite(created) && created >= cutoff; }) || null;
}
...
function liveMessageUpdateDue(account, previous, checked, settings = {}) {
  if (checked.isLive !== true || previous.isLive !== true || !checked.event || settings.liveMessageRefreshEnabled === false) return false;
  const rawLast = previous.lastLiveMessageUpdatedAt || previous.lastLiveMessageUpdateAt || 0;
  const numericLast = Number(rawLast); const parsedLast = Number.isFinite(numericLast) ? numericLast : Date.parse(String(rawLast));
  if (!Number.isFinite(parsedLast)) return true;
  const requested = Number(settings.liveMessageRefreshMs); const refreshMs = LIVE_MESSAGE_REFRESH_INTERVALS.has(requested) ? requested : LIVE_MESSAGE_REFRESH_MS;
  return Date.now() - parsedLast >= refreshMs;
}

async function forcePostCreatorLive(client, guildId, creatorId, options = {}) {
  const config = configFor(guildId, options.guildConfig); const creator = config.creators?.[creatorId];
  ...
  const sent = [];
  for (const account of liveAccounts) { const event = { ...account.state.lastLiveEvent, type: 'live' }; const delivered = await sendAlert(client, guildId, config, account, event, options); sent.push({ accountId: account.accountId, platform: account.platform, ...delivered }); }
...
async function checkGuildAccounts(client, guildId, options = {}) {
  ...
  const quiet = quietHoursActive(config.settings); const monitorUpdates = new Map(); const duplicateMerges = new Map(); const analyticsDelta = {}; const historyEntries = []; const results = [];
  ...
  for (const accountId of accountIds) {
    ...
    const runKey = `${guildId}:${accountId}`;
    if (runningGuilds.has(runKey)) continue;
    runningGuilds.add(runKey);
    try {
      ...
      if (checked.isLive === true) {
        const incomingEventId = checked.event?.id ? String(checked.event.id) : null;
        const previousEventId = previous.liveEventId ? String(previous.liveEventId) : null;
        const newBroadcast = Boolean(incomingEventId && previousEventId && incomingEventId !== previousEventId);
        state.isLive = true;
        state.liveEventId = incomingEventId || state.liveEventId || null;
        state.liveStartedAt = checked.event?.startedAt || (newBroadcast ? null : state.liveStartedAt) || null;
        state.lastLiveEvent = checked.event || state.lastLiveEvent || null;
        if (newBroadcast) state.peakViewers = 0;
        const viewers = Number(checked.event?.viewerCount);
        if (Number.isFinite(viewers) && viewers >= 0) state.peakViewers = Math.max(Number(state.peakViewers || 0), viewers);
      } else if (checked.isLive === false) {
        state.isLive = false;
        if (previous.isLive === true) state.lastLiveEndedAt = checked.checkedAt || now();
      }
      const creator = creatorFor(config, accountId); const duplicateIds = resolvedDuplicateIds(config, account, checked, creator); for (const duplicateId of duplicateIds) duplicateMerges.set(duplicateId, accountId);
      const delivered = []; const events = eventCandidates(account, previous, checked);
      for (const event of events) {
        const key = eventKey(event);
        if (config.settings.suppressDuplicates !== false && previous.lastAlertKey === key && event.type !== 'ended' && hasTrackedLiveMessage(previous)) continue;
        if (quiet && !options.manual && event.type !== 'ended') continue;
        if (event.type === 'ended' && previous.isLive === true) {
          let updated = null;
          if (config.settings.deleteEndedNotifications !== false) await deleteEndedAlert(client, guildId, previous).catch(() => false);
          else if (config.settings.editLiveNotifications !== false) updated = await updateEndedAlert(client, guildId, config, account, event, previous).catch(() => null);
          if (updated) delivered.push({ type: 'ended', id: event.id, ...updated });
          state.lastAlertKey = key; state.lastAlertAt = now(); state.lastLiveMessageId = null; state.lastLiveMessageChannelId = null; continue;
        }
        const delivery = await sendAlert(client, guildId, config, account, event, options);
        delivered.push({ type: event.type, id: event.id, ...delivery }); state.lastAlertKey = key; state.lastAlertAt = now(); state.lastAlertMessageId = delivery.messageId; state.lastAlertChannelId = delivery.channelId;
        if (event.type === 'live') { state.lastLiveMessageId = delivery.messageId; state.lastLiveMessageChannelId = delivery.channelId; state.lastLiveMessageUpdatedAt = now(); }
      }
      if (liveMessageUpdateDue(account, previous, checked, config.settings)) {
        let updated = config.settings.editLiveNotifications === false ? null : await updateLiveAlert(client, guildId, config, account, checked.event, previous).catch(() => null);
        let recovered = false;
        if (!updated && config.settings.editLiveNotifications !== false && checked.isLive === true && checked.event && hasTrackedLiveMessage(previous)) {
          updated = await sendAlert(client, guildId, config, account, checked.event, { ...options, suppressMention: true }).catch(() => null);
          recovered = Boolean(updated);
        }
        if (updated) {
          state.lastAlertKey = eventKey(checked.event); state.lastAlertMessageId = updated.messageId; state.lastAlertChannelId = updated.channelId; state.lastLiveMessageId = updated.messageId; state.lastLiveMessageChannelId = updated.channelId; state.lastLiveMessageUpdatedAt = now();
          delivered.push({ type: 'live', id: checked.event.id, refreshed: !recovered, recovered, ...updated });
        }
      }
      ...
      analyticsDelta.checks = Number(analyticsDelta.checks || 0) + 1; if (delivered.length) analyticsDelta.alerts = Number(analyticsDelta.alerts || 0) + delivered.length;
      for (const item of delivered) historyEntries.push({ id: `history_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`, createdAt: now(), accountId, platform: account.platform, status: item.recovered ? 'alert_recovered' : item.refreshed ? 'alert_updated' : 'alert_sent', alertType: item.type, eventId: item.id, messageId: item.messageId, channelId: item.channelId });
    } catch (error) {
...
function startupSocialStudio(client) {
  ...
  const run = () => { for (const guild of client.guilds.cache.values()) checkGuildAccounts(client, guild.id).catch((error) => console.error(`[Social Studio] monitor failed for guild ${guild.id}:`, error)); };
  const initial = setTimeout(run, 5000); initial.unref?.(); timer = setInterval(run, interval); timer.unref?.(); console.log(`✅ Social Studio monitor started (${interval}ms)`); return timer;
}
