...
} = require('./socialStudioLiveRefreshPolicy');
...
function projectLiveRefreshState(account, history = [], settings = {}) {
  ...
  const hasDedicatedLiveMessage = state.isLive === true && Boolean(
    state.lastLiveMessageId && state.lastLiveMessageChannelId
  );

  const legacyLiveMessage = state.isLive === true
  ...
  const hasPersistedLiveMessage = hasDedicatedLiveMessage || Boolean(
    legacyLiveMessage.lastLiveMessageId && legacyLiveMessage.lastLiveMessageChannelId
  );

  const liveHistory = !hasPersistedLiveMessage
    ? [...(Array.isArray(history) ? history : [])].reverse().find((entry) =>
      entry?.accountId
      && String(entry.accountId) === String(account.accountId)
  ...
  const hasTrackedLiveMessage = effectiveState.isLive === true && Boolean(
  ...
  const alertTypes = Array.isArray(account.alertTypes)
    ? account.alertTypes.filter((type) => !(
      hasTrackedLiveMessage && String(type).toLowerCase() === 'ended'
    ))
    : account.alertTypes;

  const projectedTimestamp = hasTrackedLiveMessage
    ? projectedRefreshTimestamp(account, effectiveState, settings)
    : null;
...
function projectGuildConfig(guildConfig) {
  ...
  const modules = guildConfig.modules && typeof guildConfig.modules === 'object'
    ? guildConfig.modules
    : {};
  ...
  const projectedAccounts = Object.fromEntries(
    Object.entries(effectiveAccounts && typeof effectiveAccounts === 'object' ? effectiveAccounts : {})
      .map(([accountId, account]) => [
        accountId,
        projectLiveRefreshState(account, history, settings),
      ])
  );
  ...
  return {
    ...
    modules: {
      ...modules,
      social: { ...social, accounts: projectedAccounts },
    },
  };
}
