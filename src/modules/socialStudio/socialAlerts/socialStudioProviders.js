...
function providerInfo(platform) {
  ...
  if (!provider || !capability) return { id, label: id || 'Unknown', providerClass: 'unsupported', supportedAlertTypes: [], status: 'unsupported', configured: false, authorizationRequired: false, productionSupported: false };
  const configured = provider.isConfigured();
  const supportedAlertTypes = Array.isArray(provider.alertTypes) && provider.alertTypes.length ? [...provider.alertTypes] : [...capability.alertTypes];
  return { id: provider.id, label: provider.label, providerClass: capability.providerClass, supportedAlertTypes, status: configured ? 'ready' : 'configuration_required', configured, authorizationRequired: capability.providerClass === 'official_api' && !configured, productionSupported: true };
}
...
function diagnosticHealth(diagnostic = {}) {
  ...
  if (status === 'unsupported' || failureCategory === 'unsupported') return { level: 'unsupported', healthy: false, actionable: false, label: 'Unsupported' };
  if (status === 'configuration_required' || failureCategory === 'configuration') return { level: 'configuration', healthy: false, actionable: true, label: 'Configuration Required' };
  if (failureCategory || ['timeout', 'unavailable', 'error', 'failed'].includes(status)) return { level: 'error', healthy: false, actionable: true, label: 'Provider Error' };
  if (diagnostic.deliveryReady === false) return { level: 'delivery', healthy: false, actionable: true, label: 'Delivery Setup Required' };
...
function applyDiagnosticState(account = {}, diagnostic = {}) {
  ...
  return { ...account, diagnostics: { ...(account.diagnostics && typeof account.diagnostics === 'object' ? account.diagnostics : {}), provider: record } };
}
function summarizeDiagnostics(diagnostics = []) {
  const summary = { total: 0, healthy: 0, live: 0, offline: 0, configuration: 0, providerErrors: 0, deliveryIssues: 0, unsupported: 0, slow: 0, maxLatencyMs: 0, overall: 'healthy' };
  for (const diagnostic of Array.isArray(diagnostics) ? diagnostics : []) {
    ...
    summary.total += 1; summary.maxLatencyMs = Math.max(summary.maxLatencyMs, latencyMs);
    if (latencyMs >= 5000) summary.slow += 1;
    if (diagnostic?.isLive === true) summary.live += 1; else if (diagnostic?.isLive === false) summary.offline += 1;
    if (health.level === 'healthy') summary.healthy += 1; else if (health.level === 'configuration') summary.configuration += 1; else if (health.level === 'delivery') summary.deliveryIssues += 1; else if (health.level === 'unsupported') summary.unsupported += 1; else summary.providerErrors += 1;
  }
  if (summary.providerErrors > 0) summary.overall = 'error'; else if (summary.configuration > 0 || summary.deliveryIssues > 0) summary.overall = 'attention'; else if (summary.total === 0) summary.overall = 'empty';
  return summary;
}
async function runProviderCheck(provider, account, platform) {
  const timeoutMs = providerTimeoutMs(); let timer = null;
...
function resolveAccountIdentity(account = {}, checked = {}) {
  return { username: checked.resolvedUsername || account.username || account.normalizedUsername || null, resolvedUsername: checked.resolvedUsername || account.normalizedUsername || account.username || null, externalId: checked.externalId || account.externalId || null, displayName: checked.displayName || account.displayName || null, profileUrl: checked.url || checked.profileUrl || account.profileUrl || account.url || null };
}
