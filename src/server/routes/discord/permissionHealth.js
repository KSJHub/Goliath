...
// src/server/routes/discord/permissionHealth.js
...
const formsStore = require('../../../modules/feedbackStudio/forms/forms');
...
function failure(res, error, status = 500) { console.error('[PermissionHealth API]', error); return res.status(status).json({ success: false, error: error.message || 'Permission health check failed.' }); }
function cleanDiscordId(value, label = 'Discord ID') { const id = String(value || '').replace(/\D/g, ''); if (!id || id.length < 15) throw new Error(`Invalid ${label}.`); return id; }
function cleanId(value) { const id = String(value || '').replace(/[<@#!&>]/g, '').trim(); return /^\d{15,25}$/.test(id) ? id : null; }
function uniqueIds(values = []) { return [...new Set((Array.isArray(values) ? values : [values]).map(cleanId).filter(Boolean))]; }
function uniqueFromObjects(items = [], getter) { return uniqueIds(items.flatMap((item) => getter(item) || [])); }
...
async function getGuild(req, guildId) { const client = getDiscordClient(req); const cachedGuild = client?.guilds?.cache?.get?.(guildId); if (cachedGuild) return cachedGuild; const fetchedGuild = typeof client?.guilds?.fetch === 'function' ? await client.guilds.fetch(guildId).catch(() => null) : null; if (!fetchedGuild) throw new Error('Guild is not available to the Discord client.'); return fetchedGuild; }
function summariseGuardResult(result) { return { ok: result.ok, scope: result.scope, channelId: result.channelId, channelName: result.channelName, missingPermissions: result.missingPermissions || [], failures: result.failures || [], autoFixAvailable: result.autoFixAvailable === true, message: result.message }; }
function getSectionStatus(issueCount = 0, configuredCount = 0) { if (issueCount > 0) return 'critical'; if (configuredCount > 0) return 'healthy'; return 'idle'; }
function scoreFromCounts({ basePermissionIssueCount = 0, channelIssueCount = 0, roleIssueCount = 0, moduleIssueCount = 0 }) { const penalty = (basePermissionIssueCount * 14) + (roleIssueCount * 4) + (channelIssueCount * 3) + (moduleIssueCount * 2); return Math.max(0, Math.min(100, 100 - penalty)); }
function scoreStatus(score) { if (score >= 90) return 'healthy'; if (score >= 70) return 'warning'; return 'critical'; }
function buildCategory(key, label, issueCount, checkedCount, description, recommendations = []) { return { key, label, status: issueCount > 0 ? 'critical' : checkedCount > 0 ? 'healthy' : 'idle', issueCount, checkedCount, description, recommendations: recommendations.filter(Boolean) }; }
function buildModuleDiagnostic({ key, label, configuredCount, channelIds = [], roleIds = [], channelIssueMap, roleIssueMap, notes = [] }) { const channelIssues = uniqueIds(channelIds).map((id) => channelIssueMap.get(id)).filter(Boolean); const roleIssues = uniqueIds(roleIds).map((id) => roleIssueMap.get(id)).filter(Boolean); const issueCount = channelIssues.length + roleIssues.length; return { key, label, status: getSectionStatus(issueCount, configuredCount), configuredCount, issueCount, channelIssueCount: channelIssues.length, roleIssueCount: roleIssues.length, channelIds: uniqueIds(channelIds), roleIds: uniqueIds(roleIds), channelIssues, roleIssues, notes: notes.filter(Boolean), recommendation: issueCount ? 'Fix the affected channel permissions or move Goliath above the affected roles.' : configuredCount ? 'No configured permission issues found for this module.' : 'No active configuration found yet.' }; }

async function checkGuildBasePermissions(guild) {
  const botMember = await getBotMember(guild);
  if (!botMember) return { ok: false, missingPermissions: [], message: 'Goliath could not read its own server member profile.' };
  const required = [PermissionFlagsBits.ManageChannels, PermissionFlagsBits.ManageRoles, PermissionFlagsBits.ManageMessages, PermissionFlagsBits.ViewAuditLog];
  const recommended = [PermissionFlagsBits.ManageWebhooks, PermissionFlagsBits.ModerateMembers, PermissionFlagsBits.SendMessages, PermissionFlagsBits.EmbedLinks, PermissionFlagsBits.AttachFiles, PermissionFlagsBits.ReadMessageHistory];
  ...
  return { ok: missingPermissions.length === 0, missingPermissions, missingRecommendedPermissions, botRoleId: botMember.roles?.highest?.id || null, botRoleName: botMember.roles?.highest?.name || null, botRolePosition: botMember.roles?.highest?.position || null, message: missingPermissions.length ? 'Goliath is missing one or more required server-level permissions.' : missingRecommendedPermissions.length ? 'Goliath has required permissions but is missing recommended permissions.' : 'Goliath has the recommended server-level permissions.' };
}

async function checkChannels(guild, limit = 50) {
  const channels = [...(guild.channels?.cache?.values?.() || [])].filter((channel) => channel?.isTextBased?.() || channel?.type === 4).slice(0, Math.max(1, Math.min(Number(limit) || 50, 200)));
  const checks = [];
  for (const channel of channels) { const result = await validateChannelAccess(guild, channel.id, DEFAULT_BOT_CHANNEL_PERMISSIONS, { scope: 'permission_health.channel' }); if (!result.ok) checks.push({ type: 'channel', channelId: channel.id, channelName: channel.name, channelType: channel.type, result: summariseGuardResult(result) }); }
  return { checked: channels.length, issueCount: checks.length, issues: checks };
}

async function checkRoles(guild, limit = 100) {
  const roles = [...(guild.roles?.cache?.values?.() || [])].filter((role) => role && role.id !== guild.id).sort((a, b) => Number(b.position || 0) - Number(a.position || 0)).slice(0, Math.max(1, Math.min(Number(limit) || 100, 250)));
  const issues = []; const dangerousRoles = [];
  for (const role of roles) { const result = await canManageRole(guild, role.id); if (!result.ok && ['role_hierarchy', 'missing_manage_roles', 'managed_role'].includes(result.reason)) issues.push({ type: 'role', roleId: role.id, roleName: role.name, rolePosition: role.position, reason: result.reason, message: result.message, fix: result.fix || null }); if (role.permissions?.has?.(PermissionFlagsBits.Administrator) && !role.managed) dangerousRoles.push({ roleId: role.id, roleName: role.name, rolePosition: role.position, reason: 'administrator_permission', message: 'This role has Administrator permission.' }); }
...
function buildModuleDiagnostics(guildId, channelHealth, roleHealth) {
  ...
  const formsSection = formsStore.getFormsSection(guildId);
  ...
  const formList = Object.values(formsSection.forms || {});
  const formPanels = Object.values(formsSection.panels || {});
  ...
  return { sectionCount: sections.length, issueCount: sections.reduce((total, section) => total + section.issueCount, 0), configuredCount: sections.reduce((total, section) => total + section.configuredCount, 0), sections };
}
...
router.get('/:guildId', async (req, res) => {
  try {
    ...
    const [basePermissions, channelHealth, roleHealth] = await Promise.all([checkGuildBasePermissions(guild), checkChannels(guild, req.query.channelLimit), checkRoles(guild, req.query.roleLimit)]);
    ...
    return success(res, { guildId, checkedAt: new Date().toISOString(), status: scoreStatus(healthScore), healthScore, summary: { issueCount, warningCount, basePermissionIssueCount, recommendedPermissionWarningCount: (basePermissions.missingRecommendedPermissions || []).length, channelIssueCount: channelHealth.issueCount, roleIssueCount: roleHealth.issueCount, dangerousRoleCount: roleHealth.dangerousRoleCount || 0, moduleIssueCount: modules.issueCount, moduleConfiguredCount: modules.configuredCount }, categories, recommendations: categories.flatMap((category) => category.recommendations || []).slice(0, 12), basePermissions, channels: channelHealth, roles: roleHealth, modules });
