...
const MODULE_CATALOG = Object.freeze({
  verification: {
    key: 'verification', name: 'Verification', icon: '\u2705', category: 'Security',
    summary: 'Verify members, assign roles and deploy a custom verification panel.',
    apiBase: '/api/verification', dashboardPath: '/modules/securityStudio/verification', maturity: 'in_progress', configurable: true,
  },
  autoRoles: {
    key: 'autoRoles', name: 'Auto Roles', icon: '\uD83E\uDD16', category: 'Automation',
    summary: 'Automatically assign roles to members and bots when they join.',
    apiBase: '/api/modules/:guildId/auto-roles', dashboardPath: '/modules/auto-roles', maturity: 'in_progress', configurable: true,
  },
  embedStudio: {
...
function failure(res, error, status = 500) {
  ...
  if (isGoliathPermissionError(error)) {
    ...
    return res.status(403).json({
      success: false, code: error.code, error: error.message, message: details.message || error.message,
      scope: details.scope || null, guildId: details.guildId || null, failures: details.failures || [],
      missingPermissions: details.missingPermissions || [], metadata: details.metadata || {},
      autoFixAvailable: Boolean(details.autoFixAvailable), confirmationRequired: Boolean(details.confirmationRequired),
    });
  }
...
function cleanModuleKey(value) { const key = String(value || '').trim(); if (!/^[a-zA-Z0-9_-]{2,80}$/.test(key)) throw new Error('Invalid module key.'); return key; }
function cleanPresetName(value) { const name = String(value || '').trim().slice(0, 50); if (!name) throw new Error('Preset name is required.'); return name; }
function cleanDeploymentKey(value) { const key = String(value || '').trim().slice(0, 100); if (!key) throw new Error('Deployment key is required.'); return key; }
function countEmbedPresetsForLimit(req) { const guildId = getGuildId(req); const name = cleanPresetName(req.body?.name); const presets = getGuildSection(guildId, 'embedPresets', {}); return Object.keys(presets || {}).filter((key) => key !== 'updatedAt' && key !== name).length; }
...
function normalizeModuleMap(modules = {}) {
  ...
  if (modules && typeof modules === 'object' && !Array.isArray(modules)) {
    for (const [key, value] of Object.entries(modules)) {
      ...
      else if (typeof value === 'boolean') output[key] = { enabled: value !== false };
      else output[key] = { enabled: true };
    }
  }
  for (const [key, meta] of Object.entries(MODULE_CATALOG)) output[key] = { ...(output[key] || {}), enabled: output[key]?.enabled === undefined ? false : output[key].enabled !== false, meta };
  return output;
}
...
async function guardManageableRoles(guild, roleIds = [], scope = 'roles') {
  const cleanRoleIds = autoRoleStore.cleanRoleIds(roleIds); if (!cleanRoleIds.length) return null;
  const result = await validateRoleSelection(guild, cleanRoleIds, { scope, requireManageable: true }); if (!result.ok) throw result.toError(); return result;
}
async function guardAutoRoleConfig(req, guildId, input = {}) {
  const roleIds = [...(Array.isArray(input.joinRoles) ? input.joinRoles : []), ...(Array.isArray(input.botRoles) ? input.botRoles : [])];
  if (!roleIds.length) return null; const guild = await fetchGuild(req, guildId); if (!guild) throw new Error('Guild is unavailable.'); return guardManageableRoles(guild, roleIds, 'auto_roles.config_roles');
}
async function guardVerificationRoles(req, guildId, input = {}) {
  ...
  const roleIds = [settings?.verifiedRoleId, settings?.unverifiedRoleId, ...(Array.isArray(settings?.verifiedRoleIds) ? settings.verifiedRoleIds : []), ...(Array.isArray(settings?.pendingRoleIds) ? settings.pendingRoleIds : [])].filter(Boolean);
  if (!roleIds.length) return null; const guild = await fetchGuild(req, guildId); if (!guild) throw new Error('Guild is unavailable.'); return guardManageableRoles(guild, roleIds, 'verification.roles');
}

function getEmbedStudioPayload(guildId) {
  const builder = getGuildSection(guildId, 'embedBuilder', { draft: {}, templates: {}, deployments: {} });
  const presets = getGuildSection(guildId, 'embedPresets', {}); const deployments = getAllEmbedDeployments(guildId);
...
function getVerificationPayload(guildId) {
  const status = verificationManager.getVerificationStatus(guildId); const section = verificationStore.getVerificationSection(guildId); const panels = Object.values(section.panels || {});
...
router.post('/:guildId/embed-studio/presets', requirePlanLimit('embedPresets', countEmbedPresetsForLimit), (req, res) => {
  try {
    const guildId = getGuildId(req); const name = cleanPresetName(req.body?.name);
    const preset = saveEmbedPreset(guildId, name, req.body?.embed ? { ...req.body.embed, content: req.body.content || '' } : req.body || {});
    const template = embedTemplateManager.saveTemplate(guildId, { templateId: name, name, module: req.body?.module || 'global', templateType: req.body?.templateType || req.body?.module || 'global', content: req.body?.content || '', embed: req.body?.embed || req.body || {}, tags: req.body?.tags || [] });
